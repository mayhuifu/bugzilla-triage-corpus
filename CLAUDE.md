...
**Schema contract with the desktop app.** `meta.schemaVersion = "2"` (or `"2-no-vec"` if embeddings are absent). Consumers MUST read this to decide which retrieval path to use. `meta.embeddingModel` / `embeddingDim` / `embeddingDtype` describe the vector format so the desktop's bundled query-time embedder can hard-fail on mismatch instead of returning silently-wrong results.

**Schema additions to remember.** `clauses` gained `path TEXT`, `tables_json TEXT`, `figures_json TEXT`, `mentions_json TEXT`. FTS5 widened to index `parent_title` and `path` alongside `citation` / `title` / `text`. New tables: `clauses_vec`, `parents`, `parent_vec`, `acronyms`, `eval_queries`.
